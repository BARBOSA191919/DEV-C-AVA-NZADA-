...
void mostrar_todos_estudiante(void); // pendiente 
void sacar_estudiante(void);  // pendiente

void mostrar_est_Aprobados_salon(void);
void mostrar_est_Reprobados_salon(void);
void mostrar_notas_estudiantes(void) ;
...
void menu(void)
{
	...
	while ( opc != 0)
	{
		...
		cout<<"\n 10 Mostrar Estudiantes Aprobados(salon)";
		cout<<"\n 11 Mostrar Estudiantes REprobados(salon)";
		cout<<"\n 12 Mostrar notas de forma descendente (estudiante)";
		cout<<"\n 5 compara CODIGO";
		cout<<"\n 0 Terminar";
		cout<<"\n Ingrese Opcion";
		cin>>opc;
		...
		switch(opc)
		{
			...
			case 9:	
				...
				break;	
			case 10:	
				mostrar_est_Aprobados_salon();
				break;
			case 11:	
				mostrar_est_Reprobados_salon();
				break;	
				case 12:	
				mostrar_notas_estudiantes();
				break;
			default:
				cout <<"\n Opcion no Valida !! ";
				break;
...
void mostrar_todos_estudiante(void)
{
   ...
   else
   {
   	  ...
   	  {
   	cout <<"\n"<<"Nombre del salon : "<<nuevo->salon;  //le agregamos el cin indicado 
   ...
   espera();   
	...
	// recorrer la lista de los salones y mostrar el nombre del salon
	// y mostrar todos los estudiantes de dicho salon
	
void sacar_estudiante(void)
{

int cod_borra, sw;
	lista *bus1, *bus2; // lista correspodiente para sacar el estudiante con el salon
	if (!cab)
   	{
      cout<<"no hay datos o la lista no esta creada";
	  getch();	
	   ...
	   if ( cab->codigo == cod_borra )
	   {
	   		...
	   		cout <<"\n"<<"Nombre estudiante : "<<nuevo_est->nombre; // le agreamos la variable del estudiante que es nuevo_est
         	cout <<"\n"<<"Codigo estudiante  : "<<nuevo_est->codigo; // lo mismo pero con codigo 
   		...
   		{
   			bus1 = new(lista); // un else para agregar la lista de bus 1 y bus 2
		  	bus2 = new(lista);
		  	sw = 0;
		  	bus1 = cab;
		  	bus2 = cab->ap1;
		  	while (sw == 0 && bus2 != NULL ) // el sw es igual a cero pa que no se active y bus2 diferente de null
		  	{
		  		if ( bus2->codigo == cod_borra )
				{
					sw = 1;
				...
				else
				{
					...
					bus1 = bus2;
					bus2 = bus2->ap1; // asiganmos la variable bus1 y bus 2 y utilizamos el puntador
					bus2 = bus2->ap2;	
				cout<< "este codigo no existe "; // si se digita otro codigo que no exista muestra en pantalla
				}	
			}
			if ( sw == 0 )
			{
	...
	// digitar el codigo del estudiante y buscarlo en todos los salones
	// si lo encuentra retirarlo y mostrar de que salon se retiro
	// si no se encuentra mostrar un mensaje que este codigo no existe
...
void mostrar_notas_estudiantes(void) //descendente
{
	 if (!cab)
   {
      cout<<"no hay datos o la lista no esta creada";
   ...
   else
   {
   	...
   	cout <<"\n"<<"Nombre> del salon : "<<nuevo->salon;  //le agregamos el cin indicado 
   	cout <<"\n"<<"Nombre estudiante : "<<nuevo_est->nombre;  //le agregamos el cin indicado 
   	cout <<"\n"<<"Codigo estudiante  : "<<nuevo_est->codigo; //codigo 
   	cout<<"\nNota Estudiante : "<<nuevo_est->nota<<endl;  // nota descendente
		 nuevo_est= nuevo_est->ape;	// nuevo_est lo apuntamos ape que es el correspodiente al estudiante
   }                   
   espera();   
}
